...
def merge_same_line_boxes(text_boxes, image_width, image_height,
                          max_gap_ratio=1.20, min_gap=14,
                          row_center_ratio=0.50, row_overlap_ratio=0.20,
                          crop_padding_ratio=0.05, min_crop_padding=3,
                          max_crop_padding=14, min_keep_area_ratio=0.0006,
                          min_keep_height_ratio=0.018):
    """Merge fragmented boxes only inside a strictly separated text row.

    This variant is intended for small, dense labels. It avoids the old
    chain-merge behavior where a growing box can absorb adjacent rows.
    """
    if not text_boxes:
        return []

    items = []
    min_keep_area = max(8, int(image_width * image_height * min_keep_area_ratio))
    min_keep_height = max(5, int(image_height * min_keep_height_ratio))
    for box in text_boxes:
        ...
        if box_area < min_keep_area or box_h < min_keep_height:
            continue
        items.append({
            'rect': rect,
            'cx': (x1 + x2) / 2.0,
    ...
    heights = [item['h'] for item in items]
    median_h = float(np.median(heights)) if heights else 1.0
    row_center_limit = max(3.0, median_h * row_center_ratio)
    rows = []

    for item in sorted(items, key=lambda value: value['cy']):
        assigned_row = None
        for row in rows:
            rx1, ry1, rx2, ry2 = row['bounds']
            ix1, iy1, ix2, iy2 = item['rect']
            row_h = max(1, ry2 - ry1)
            item_h = max(1, iy2 - iy1)
            vertical_overlap = max(0, min(ry2, iy2) - max(ry1, iy1))
            overlap = vertical_overlap / float(min(row_h, item_h))
            center_close = abs(item['cy'] - row['center_y']) <= row_center_limit

            if center_close or overlap >= row_overlap_ratio:
                assigned_row = row
                break
        ...
        if assigned_row is None:
            ...
            assigned_row['items'].append(item)
            centers = [value['cy'] for value in assigned_row['items']]
            assigned_row['center_y'] = float(np.median(centers))
            x1, y1, x2, y2 = assigned_row['bounds']
            ox1, oy1, ox2, oy2 = item['rect']
            assigned_row['bounds'] = [
                min(x1, ox1),
                min(y1, oy1),
                max(x2, ox2),
    ...
    for row in sorted(rows, key=lambda value: value['center_y']):
        ...
        for item in row_items:
            ...
            overlap = vertical_overlap / float(min(current_h, other_h))
            horizontal_gap = ox1 - x2
            gap_limit = max(min_gap, median_h * max_gap_ratio)

            if horizontal_gap <= gap_limit and overlap >= row_overlap_ratio:
                current = [
                    min(x1, ox1),
                    min(y1, oy1),
                    max(x2, ox2),
    ...
    for rect in merged_rects:
        ...
        pad_y = _clamp_padding(rect_h, crop_padding_ratio * 1.4, min_crop_padding, max_crop_padding)
        padded_rect = [
            max(0, int(x1 - pad_x)),
            max(0, int(y1 - pad_y)),
        ...
        result_boxes.append(_rect_to_points(padded_rect, image_width, image_height))
