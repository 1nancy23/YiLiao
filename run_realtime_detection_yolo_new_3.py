...
from src.segmentation.segmenter_yolo import YOLOTileProcessor
from src.processor.img_cropper import crop_image_by_coordinates
from src.stream.video_stream import USBVideoStream, VideoStream
from src.utils.img_utils import ImageProcessor
...
def run_realtime_detection(
        ...
        frame_callback=None,
        yolo_model_path="./model_yolo_0615.rknn",
        yolo_input_size=640,
        ...
        video_source="rtsp",
        usb_device="/dev/video74",
        usb_width=3840,
        usb_height=2160,
        usb_capture_fps=30,
        usb_warmup_frames=25,
        usb_controls=None,
):
    import gc
    import builtins
    ...
    if not single_image_path:
        ...
        if video_path:
            ...
            selected_source = str(video_source or "rtsp").strip().lower()
            if selected_source == "usb":
                print(
                    "USB camera: {} {}x{}@{} MJPG".format(
                        usb_device,
                        int(usb_width),
                        int(usb_height),
                        float(usb_capture_fps),
                    )
                )
                video_stream = USBVideoStream(
                    usb_device,
                    target_fps=target_fps,
                    max_buffer_size=max(2, int(batch_frames or 1) + 1),
                    width=usb_width,
                    height=usb_height,
                    capture_fps=usb_capture_fps,
                    warmup_frames=usb_warmup_frames,
                    controls=usb_controls,
                )
                video_stream.start()
                ...
                raise ValueError("Unsupported video source: {}".format(selected_source))
    # ============================================================
    # YOLO 处理器
    # ============================================================
    ...
    if not single_image_path and basket_model_path:
        ...
        basket_trigger = BasketAutoTrigger(
            ...
            stable_frames_required=basket_stable_frames,
        )
    ...
    def process_batch_in_background(snapshot_frames, snapshot_predictions, keep_payload=False, save_source_frame=False):
        """
        后台线程处理流程：
        1. 选择最清晰帧
        2. 裁剪药袋、药瓶、输液袋
        ...
        try:
            ...
            def crop_text_quality(crop):
                ...
                exposure_factor = max(0.35, 1.0 - clipped * 3.0)
                return focus * exposure_factor * (0.75 + min(contrast, 64.0) / 256.0)

            def frame_quality_score(frame, candidate_detections):
                ...
                groups = (candidate_bags, candidate_bottles, candidate_shuyes)
                qualities = [
                    crop_text_quality(crop)
                    for crops in groups
                    for crop in crops
                    if crop is not None and crop.size > 0
                ]
                counts = tuple(len(crops) for crops in groups)
                category_count = sum(1 for count in counts if count > 0)
                confidence_sum = sum(float(det[4]) for det in candidate_detections)
                return (
                    counts[0] > 0,
                    category_count,
                    sum(counts),
                    min(qualities) if qualities else -1.0,
                    sum(qualities) / len(qualities) if qualities else -1.0,
                    confidence_sum,
                )
            ...
            for candidate_batch_idx, (candidate_frames, candidate_predictions) in enumerate(
                ...
                for candidate_frame_idx, (candidate_frame, candidate_detections) in enumerate(
                    ...
                    all_pairs.append((candidate_batch_idx, candidate_frame_idx, candidate_frame, candidate_detections))
            all_pairs.reverse()
            for candidate_batch_idx, candidate_frame_idx, candidate_frame, candidate_detections in all_pairs:
                frame_score = frame_quality_score(candidate_frame, candidate_detections)
                if best_frame_score is None or frame_score > best_frame_score:
            ...
            print(
                f"最清晰帧: 批次 {batch_idx + 1}/{len(snapshot_frames)} "
                f"中的第 {frame_idx + 1} 张, 文字质量评分={best_frame_score}"
            )

            if save_source_frame:
                source_dir = os.path.join(os.getcwd(), "AutoTrigger_Source_Frames")
                os.makedirs(source_dir, exist_ok=True)
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                millis = int((time.time() % 1.0) * 1000)
                source_path = os.path.join(
                    source_dir,
                    f"auto_trigger_{timestamp}_{millis:03d}_batch{batch_idx + 1}_frame{frame_idx + 1}.jpg",
                )
                cv2.imwrite(source_path, best_frame)
                print(f"[Auto Trigger Source Frame] saved: {source_path}")

            # Reuse the detections that selected this exact raw frame.  Running
            # YOLO again after CLAHE can change borderline boxes and makes frame
            # selection disagree with the crops used by OCR.
            raw_recognition_frame = np.ascontiguousarray(best_frame)
            detections = snapshot_predictions[batch_idx][frame_idx]
            ...
            raw_bags, raw_bottles, raw_shuyes = processor.crop_by_class(
                raw_recognition_frame,
                detections,
                bag_class_id=1,
                bottle_class_id=0,
                shuye_class_id=2,
            )
            # Enhance only the small target crops.  This preserves the YOLO
            # image domain and avoids CLAHE over the full 4K frame.
            cropped_bags = [enhance_trigger_frame_contrast(crop) for crop in raw_bags]
            cropped_bottles = [enhance_trigger_frame_contrast(crop) for crop in raw_bottles]
            cropped_shuyes = [enhance_trigger_frame_contrast(crop) for crop in raw_shuyes]
            ...
            for i, bottle in enumerate(cropped_bottles):
                all_tasks.append((
                    ...
                    i,
                    bottle,
                    raw_bottles[i] if i < len(raw_bottles) else bottle,
                ))
        ...
        while video_stream.running and not (stop_event is not None and stop_event.is_set()):
            ...
            if active_trigger_mode == "manual":
                ...
                if auto_capture_next_frame:
                    ...
                    if len(auto_capture_candidates) >= auto_capture_required:
                        trigger_snapshot_frames = [[
                            candidate[0] for candidate in auto_capture_candidates
                        ]]
                        trigger_snapshot_preds = [[
                            candidate[1] for candidate in auto_capture_candidates
                        ]]
                        auto_capture_candidates.clear()
                        auto_capture_next_frame = False
                        should_trigger = True
            ...
            if should_trigger:
                if (not is_processing.is_set()) or (
                    trigger_label == "basket_auto"
                    and trigger_snapshot_frames is not None
                    and trigger_snapshot_preds is not None
                ):
                    is_processing.set()
                    processing_done.clear()
                    emit_status(
