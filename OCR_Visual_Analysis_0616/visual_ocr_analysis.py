...
def crop_with_boxes(image, detections, case_dir):
...
def run_yolo(image, config, args):
    from run_realtime_detection_yolo_new_3 import normalize_camera_frame_bgr
    from src.segmentation.segmenter_yolo import YOLOTileProcessor

    model_cfg = config.get("model", {})
    normalized = normalize_camera_frame_bgr(image) if not args.no_normalize else image
...
def process_ocr_and_match(crops, classifier, recognizer, matcher):
    ...
    for key, task_type in (("bottle", "bottle"), ("bag", "bag"), ("shuye", "shuye")):
        for idx, item in enumerate(crops[key]):
            tasks.append((task_type, idx, item["image"]))

    t0 = time.perf_counter()
    batch_results = recognizer.recognize_task_batch(tasks) if tasks else {}
    ocr_sec = time.perf_counter() - t0

    results = {"bottles": [], "bags": [], "infusions": [], "timing_sec": {"ocr_batch": ocr_sec}}
    cached_names = classifier.get_cached_names() if classifier is not None else []

    for idx, item in enumerate(crops["bottle"]):
        value = batch_results.get(("bottle", idx), {})
        text = value.get("text", "")
        row = {k: v for k, v in item.items() if k != "image"}
        row.update({
            "ocr_text": text,
            "det_region_count": value.get("det_region_count", 0),
            "rec_nonempty_count": value.get("rec_nonempty_count", 0),
            "classify_angle_pred": value.get("classify_angle_pred"),
            "timing": value.get("timing", {}),
        })
        if text and matcher is not None:
            t = time.perf_counter()
            row["candidates"] = matcher.match(text, match_type="bottle", threshold=50, limit=10)
            row["match_sec"] = time.perf_counter() - t
        elif text:
            row["candidates"] = cached_names
            row["match_note"] = "db_disabled_using_all_template_names"
        else:
            row["candidates"] = []
        if row["candidates"] and classifier is not None:
            image_for_cls = value.get("classify_image", item["image"])
            t = time.perf_counter()
            cls_res = classifier.classify(row["candidates"], image_for_cls, image_already_rotated=bool(value.get("classify_image_already_rotated")))
            row["classify_sec"] = time.perf_counter() - t
            row["final_medicine"] = cls_res.get("predicted_category")
            row["confidence"] = cls_res.get("confidence")
            row["top_3"] = cls_res.get("top_3")
            row["decision_reason"] = cls_res.get("decision_reason")
            row["sift_best_template"] = cls_res.get("sift_best_template")
        results["bottles"].append(jsonable(row))

    for idx, item in enumerate(crops["bag"]):
        value = batch_results.get(("bag", idx), {})
        text = value.get("text", "")
        row = {k: v for k, v in item.items() if k != "image"}
        row.update({
            "ocr_text": text,
            "name_roi_count": value.get("name_roi_count", 0),
            "classify_angle_pred": value.get("classify_angle_pred"),
            "timing": value.get("timing", {}),
        })
        if text and matcher is not None:
            t = time.perf_counter()
            row["patient_match"] = matcher.match(text, match_type="bag", threshold=50, limit=10)
            row["match_sec"] = time.perf_counter() - t
        results["bags"].append(jsonable(row))

    for idx, item in enumerate(crops["shuye"]):
        value = batch_results.get(("shuye", idx), {})
        text = value.get("text", {})
        row = {k: v for k, v in item.items() if k != "image"}
        row.update({
            "ocr_text": text,
            "candidate_roi_count": value.get("candidate_roi_count", 0),
            "timing": value.get("timing", {}),
        })
...
def main():
    ...
    try:
        ...
        for case_index, image_path in enumerate(image_paths, start=1):
            ...
            crops = crop_with_boxes(normalized, detections, case_dir)
