...
def _select_final_bottle_medicine(candidates, cls_result, min_good_matches=4, min_match_score=0.009):
    if not candidates:
...
def process_bottle(
    ...
    try:
        ...
        if precomputed_ocr is not None:
            ...
            ocr_timing = dict(precomputed_ocr.get("timing", {}) or {})
        else:
        ...
        if _looks_like_non_bottle_text(text):
            item["status"] = "skip_non_bottle_ocr"
            item["classification_method"] = "skipped"
            item["decision_reason"] = "ocr_text_looks_like_bag_or_patient_label"
            item["timing_sec"]["total"] = time.perf_counter() - total_start
            item["timing_sec"] = _round_timing(item["timing_sec"])
            return item
...
def process_bag(
    ...
    try:
        ...
        t = time.perf_counter()
        patient = [simulate_patient_name] if simulate_patient_name else drug_matcher.match(
            text,
            match_type="bag",
            threshold=50,
            limit=10,
...
def process_task_group(
    ...
    batch_ocr = {}
    use_mixed_batch = os.environ.get("YILIAO_MIXED_OCR_BATCH", "1").lower() in ("1", "true", "yes")
