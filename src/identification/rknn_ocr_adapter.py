...
BOTTLE_DET_THRESHOLD = 0.30
BAG_DET_THRESHOLD = 0.28
INFUSION_DET_THRESHOLD = 0.24
REC_MAX_LINE_RATIO = 6.0
REC_LINE_OVERLAP_RATIO = 0.08
...
class RknnOCRRecognizer:
    ...
    def _postprocess_det_candidate(self, args):
        ...
        if item.get("filter_edge_boxes") is not None:
            ...
            extract_kwargs["filter_edge_boxes"] = False
        ...
        regions, boxes = module.extract_text_regions(
            ...
            det_map,
            scale_factor=1,
            threshold=float(item.get("det_threshold", BOTTLE_DET_THRESHOLD)),
    ...
    @staticmethod
    def _ctc_token_confidence(sequence_output, pred_indices, blank_idx=0):
        sequence = np.asarray(sequence_output, dtype=np.float32)
        if sequence.ndim != 2 or sequence.shape[0] == 0:
            return 0.0
        row_sums = np.sum(sequence, axis=1)
        if (
            float(np.min(sequence)) >= 0.0
            and float(np.max(sequence)) <= 1.0001
            ...
            probabilities = exp_values / np.maximum(
        ...
        emitted = []
        previous = -1
        for step, index in enumerate(pred_indices):
            index = int(index)
            if index == blank_idx:
                previous = -1
                continue
            if index != previous and 0 <= index < probabilities.shape[1]:
                emitted.append(float(probabilities[step, index]))
            previous = index
        return float(np.mean(emitted)) if emitted else 0.0
    ...
    def _decode_mixed_rec_batch(self, start, modules, regions, rec_output, valid_count):
        ...
        for offset in range(valid_count):
            ...
            results.append({
                ...
                "confidence": self._ctc_token_confidence(
                    rec_output[offset],
                    pred_indices_batch[offset],
                ),
            })
        return results

    @staticmethod
    def _split_long_rec_region(region):
        if region is None or region.size == 0:
            return []
        height, width = region.shape[:2]
        if height <= 0 or width <= max(32, int(height * REC_MAX_LINE_RATIO)):
            return [region]

        window_width = max(32, int(round(height * REC_MAX_LINE_RATIO)))
        overlap = max(4, int(round(window_width * REC_LINE_OVERLAP_RATIO)))
        stride = max(1, window_width - overlap)
        starts = list(range(0, max(1, width - window_width + 1), stride))
        last_start = max(0, width - window_width)
        if not starts or starts[-1] != last_start:
            starts.append(last_start)
        return [region[:, start:min(width, start + window_width)].copy() for start in starts]

    @staticmethod
    def _merge_rec_segment_texts(texts):
        merged = ""
        for text in texts:
            ...
            if not merged:
                merged = text
                continue
            max_overlap = min(12, len(merged), len(text))
            overlap = 0
        ...
        for batch_images, det_outputs in zip(batch_images_list, det_outputs_list):
            ...
            for image, det_map in zip(batch_images, det_maps):
                ...
                regions, boxes = module.extract_text_regions(
                    ...
                    det_map,
                    scale_factor=1,
                    threshold=(INFUSION_DET_THRESHOLD if module is infusion_ocr else BOTTLE_DET_THRESHOLD),
                    box_padding_ratio=padding["box_padding_ratio"],
                    min_box_padding=padding["min_box_padding"],
                    max_box_padding=padding["max_box_padding"],
                    **extract_kwargs,
    ...
    def _recognize_mixed_candidates_batch(self, candidates, timing_prefix="mixed_"):
        ...
        texts_by_owner = {}
        details_by_owner = {}
        self._last_mixed_details_by_owner = details_by_owner
        if not candidates:
        ...
        for chunk, det_outputs in zip(chunks, det_outputs_list):
            ...
            for item, regions in postprocessed:
                for region_index, region in enumerate(regions):
                    segments = self._split_long_rec_region(region)
                    group_key = (item["owner"], id(item), region_index)
                    for segment in segments:
                        owners.append(item["owner"])
                        region_modules.append(item["module"])
                        region_group_keys.append(group_key)
                        flat_regions.append(segment)
        ...
        self.last_timing[f"{timing_prefix}text_regions"] = len(flat_regions)
        self.last_timing[f"{timing_prefix}long_line_extra_segments"] = max(
            0,
            len(flat_regions) - len(set(region_group_keys)),
        )
        with self._text_angle_lock:
        ...
        for item in candidates:
            ...
            details_by_owner.setdefault(item["owner"], [])
        ...
        grouped = {}
        group_order = []
        for owner, group_key, item in zip(owners, region_group_keys, rec_results):
            if item is None:
                continue
            if group_key not in grouped:
                grouped[group_key] = {"owner": owner, "items": []}
        ...
        for group_key in group_order:
            ...
            owner = group["owner"]
            text = self._merge_rec_segment_texts(
                [item.get("text", "") for item in group_items]
            ).strip()
            if text:
                texts_by_owner.setdefault(owner, []).append(text)
                weighted = [
                    (float(item.get("confidence", 0.0) or 0.0), max(1, len(str(item.get("text", "")))))
                ...
                details_by_owner.setdefault(owner, []).append({
                    "text": text,
                    "confidence": float(confidence),
                    "segments": len(group_items),
                })
        return texts_by_owner
    ...
    def _extract_infusion_blue_component_rois(self, image):
        ...
        img_h, img_w = image.shape[:2]
        # Segment on a bounded working image.  Gray-world normalization removes
        # global white-balance drift, while normalized blue chromaticity is
        # largely invariant to shadows and exposure changes.
        work_scale = min(1.0, 640.0 / float(max(img_h, img_w)))
        if work_scale < 1.0:
            work_w = max(1, int(round(img_w * work_scale)))
            work_h = max(1, int(round(img_h * work_scale)))
            work = cv2.resize(image, (work_w, work_h), interpolation=cv2.INTER_AREA)
        else:
            work = image
        work_h, work_w = work.shape[:2]

        channel_means = np.asarray(cv2.mean(work)[:3], dtype=np.float32)
        neutral = float(np.mean(channel_means))
        gains = np.clip(neutral / np.maximum(channel_means, 1.0), 0.75, 1.35)
        balanced = cv2.transform(work, np.diag(gains).astype(np.float32))

        blue, green, red = cv2.split(balanced)
        blue = blue.astype(np.int32)
        green = green.astype(np.int32)
        red = red.astype(np.int32)
        intensity = blue + green + red
        blue_excess = blue * 2 - green - red
        blue_mask = (
            (intensity >= 48)
            & (blue * 100 >= intensity * 37)
            & (blue_excess * 100 >= intensity * 10)
            & (blue * 100 >= green * 108)
            & (blue * 100 >= red * 115)
        )
        mask = np.where(blue_mask, 255, 0).astype(np.uint8)
        ...
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        components = []
        image_area = max(1, work_w * work_h)
        for contour in contours:
            x, y, w, h = cv2.boundingRect(contour)
            contour_area = cv2.contourArea(contour)
            if contour_area < max(60, image_area * 0.0015):
                continue
            if w < max(10, work_w * 0.025) or h < max(12, work_h * 0.025):
                continue
        ...
        for item in components[1:]:
            ...
            same_label_column = (
                overlap_ratio >= 0.18
                or abs(cx - main_cx) <= max(mw * 0.55, work_w * 0.08)
            )
        ...
        if selected:
            ...
            if work_scale < 1.0:
                ...
                x1 = max(0, int(np.floor(x1 * inv_scale)))
                y1 = max(0, int(np.floor(y1 * inv_scale)))
                x2 = min(img_w, int(np.ceil(x2 * inv_scale)))
                y2 = min(img_h, int(np.ceil(y2 * inv_scale)))
            roi = image[y1:y2, x1:x2]
            if roi.size > 0:
    ...
    def recognize_task_batch(self, task_group):
        ...
        for task in task_group:
            task_type, idx, crop = task[:3]
            raw_crop = task[3] if len(task) > 3 else crop
            normalized_tasks.append((task_type, idx, crop))
            raw_crops[(task_type, idx)] = raw_crop
        bottle_tasks = [task for task in normalized_tasks if task[0] == "bottle"]
        bag_tasks = [task for task in normalized_tasks if task[0] == "bag"]
        shuye_tasks = [task for task in normalized_tasks if task[0] == "shuye"]
        mixed_candidates = []
        ...
        for _, idx, crop in bottle_tasks:
            ...
            candidate = {
                ...
                "det_threshold": BOTTLE_DET_THRESHOLD,
                "split_rows": False,
            ...
            for _, idx, _crop in bag_tasks:
                ...
                def append_name_roi(raw_roi, debug_name, split_rows=True):
                    ...
                    mixed_candidates.append({
                        ...
                        "det_threshold": BAG_DET_THRESHOLD,
            ...
            for _, idx, _crop in shuye_tasks:
                ...
                for roi_name, roi in self._extract_infusion_candidate_rois(rotated_crop):
                    ...
                    mixed_candidates.append({
                        ...
                        "det_threshold": INFUSION_DET_THRESHOLD,
                        "split_rows": "infusion",
                        "merge_boxes": True,
        ...
        details_by_owner = getattr(self, "_last_mixed_details_by_owner", {}) or {}
        ...
        for _, idx, _crop in bottle_tasks:
            ...
            if cls_item is not None:
                raw_crop = raw_crops.get(("bottle", idx))
                if raw_crop is not None and raw_crop.size > 0:
                    result["classify_image"] = _reverse_rotate_with_label(
                        raw_crop,
                        cls_item["pred"],
                    )
                else:
        ...
        for _, idx, _crop in bag_tasks:
            ...
            fallback_owner = ("bag_fallback", idx)
            fallback_details = details_by_owner.get(fallback_owner, []) or [
                {"text": text, "confidence": 0.0}
                for text in texts_by_owner.get(fallback_owner, [])
            ]
            ...
            candidate_texts = sorted(
                ...
                key=lambda value: (value["confidence"], len(value["text"])),
                reverse=True,
            )
            text = candidate_texts[0]["text"] if candidate_texts else ""
            result = {
                "text": text,
