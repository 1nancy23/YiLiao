...
如果病人姓名已经存在，脚本默认拒绝重复插入；传入 ``--update-existing``
可以更新该病人的药品名称。
"""
...
def find_existing_patients(conn, settings, patient):
    """按病人姓名查询已有记录，防止同名数据被重复插入。"""
    sql = (
        f"SELECT {settings['medicines_column']} "
        f"FROM {settings['batch_table']} "
        f"WHERE {settings['patient_column']} = %s "
        "LIMIT 2"
    )
    ...
    for row in rows:
        if isinstance(row, dict):
            medicines_values.append(row[settings["medicines_column"]])
        else:
            medicines_values.append(row[0])
    return medicines_values


def insert_patient_batch(conn, settings, patient, medicine_names):
...
def update_patient_medicines(conn, settings, patient, medicine_names):
    """只根据病人名称更新药品 JSON，不涉及任何 ID 字段。"""
    sql = (
        f"UPDATE {settings['batch_table']} "
        f"SET {settings['medicines_column']} = %s "
        f"WHERE {settings['patient_column']} = %s "
        "LIMIT 1"
    )
    values = (build_medicines_json(medicine_names), patient)
    with conn.cursor() as cursor:
        cursor.execute(sql, values)
    conn.commit()
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--update-existing",
        action="store_true",
        help="病人已存在时，只更新该病人的药品名称；默认拒绝重复录入",
    )
...
def main(argv=None):
    ...
    try:
        ...
        existing_medicines = find_existing_patients(conn, settings, patient)
        if len(existing_medicines) > 1:
            print(
                f"[错误] 病人“{patient}”存在多条历史记录，请先人工清理重复数据。",
                file=sys.stderr,
            )
            return 3
        ...
        if existing_medicines:
            if not args.update_existing:
                print(f"[拒绝重复] 病人“{patient}”已存在，未写入新数据。", file=sys.stderr)
                print(f"[已有药品JSON] {existing_medicines[0]}", file=sys.stderr)
                print("如需更新药品，请增加 --update-existing。", file=sys.stderr)
                return 3

            update_patient_medicines(conn, settings, patient, medicine_names)
            print("[完成] 已根据病人名称更新药品名称。")
            return 0
